...
if { [file exists $project_dir/$project_name.prjx] } {
  ...
  import_files -sdc "${constraint_path}/riscv.sdc"

	organize_tool_files \
		-tool {PLACEROUTE} \
		-file "${project_dir}/constraint/riscv.sdc" \
  ...
  organize_tool_files -tool {VERIFYTIMING} -file "${project_dir}/constraint/riscv.sdc" -module {MPFS_DISCOVERY_KIT::work} -input_type {constraint}
...
if { $ARCHI == 32 } {
    ...
    configure_tool -name {SYNTHESIZE} \
        ...
        -params {SYNPLIFY_OPTIONS:message_override -warning DE117} \
