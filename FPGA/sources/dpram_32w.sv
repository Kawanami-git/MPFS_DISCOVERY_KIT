...
module dpram_32w #(
  ...
  wire  [           39:0] a_rdata_o_bank[NB_BANKS];
  /// Per-bank 40-bit read data (Port B)
  wire  [           39:0] b_rdata_o_bank[NB_BANKS];
  ...
  generate
    if (NB_BANKS == 1) begin : gen_single_bank
      ...
      dpram_40x1024 u_bank0 (
          ...
          .a_clk_i  (a_clk_i),
          .a_addr_i (A_ROW),
          .a_wdata_i(pack40(a_wdata_i)),
          .a_be_i   (a_be_i),
          .a_wren_i (a_wren_i),
          .a_rden_i (a_rden_i),
          .a_rdata_o(a_rdata_o_bank[0]),
          // --- Port B ---
          .b_clk_i  (b_clk_i),
          .b_addr_i (B_ROW),
          .b_wdata_i(pack40(b_wdata_i)),
          .b_be_i   (b_be_i),
          .b_wren_i (b_wren_i),
          .b_rden_i (b_rden_i),
          .b_rdata_o(b_rdata_o_bank[0])
      );
    end
  ...
  */
  assign a_rdata_o = unpack32(a_rdata_o_bank[a_bank_sel_q]);

  /*!
  * Map the per-bank 40-bit outputs to 32-bit external data using the
  * registered bank select (matches 1-cycle read latency).
  */
  assign b_rdata_o = unpack32(b_rdata_o_bank[b_bank_sel_q]);
